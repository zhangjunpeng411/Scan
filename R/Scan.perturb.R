...
IDA <- function(miRExp, mRExp,                  
    ...
    priori <- index[order(index[, 1]), ][, 3:4]
    ...
    res <- data.frame(priori, ef, ef.pvalue)    
    ...
    miRlist <- unique(priori[, 1])
    Tarlist <- unique(priori[, 2])
...
Scan.perturb <- function(miRExp, mRExp, 
     ...
     if (method == "IDA"){
        res.all <- IDA(miRExp, mRExp, priori.information, pcmethod = pcmethod, alpha = alpha, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("pcalg", "igraph", "WGCNA"), .export = c("IDA", "querydata")) %dopar% {
            IDA(miRExp[-i, ], mRExp[-i, ], priori.information, pcmethod = pcmethod, alpha = alpha, p.value = p.value)
        }     
        ...
        res.all <- Pearson(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "WGCNA"), .export = c("Pearson", "querydata")) %dopar% {
            Pearson(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Spearman(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "WGCNA"), .export = c("Spearman", "querydata")) %dopar% {
            Spearman(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Kendall(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "WGCNA"), .export = c("Kendall", "querydata")) %dopar% {
            Kendall(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Dcor(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "energy"), .export = c("Dcor", "querydata")) %dopar% {
            Dcor(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- RDC(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = "igraph", .export = c("RDC", "rdc_generic", "querydata")) %dopar% {
            RDC(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Hoeffd(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "Hmisc"), .export = c("Hoeffd", "querydata")) %dopar% {
            Hoeffd(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Zscore(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = "igraph", .export = c("Zscore", "querydata")) %dopar% {
            Zscore(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- MI(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "parmigene"), .export = c("MI", "mi_generic", "querydata", "matrixzscore")) %dopar% {
            MI(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- MIC(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "minerva"), .export = c("MIC", "mic_generic", "querydata", "matrixzscore")) %dopar% {
            MIC(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Lasso(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "glmnet"), .export = c("Lasso", "lasso_generic", "querydata", "matrixzscore")) %dopar% {
            Lasso(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Elastic(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "glmnet"), .export = c("Elastic", "elastic_generic", "querydata", "matrixzscore")) %dopar% {
            Elastic(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Ridge(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "glmnet"), .export = c("Ridge", "ridge_generic", "querydata", "matrixzscore")) %dopar% {
            Ridge(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Euclidean(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "stats", "pracma"), .export = c("Euclidean", "euclidean_generic", "querydata", "matrixzscore")) %dopar% {
            Euclidean(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Manhattan(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "stats", "pracma"), .export = c("Manhattan", "manhattan_generic", "querydata", "matrixzscore")) %dopar% {
            Manhattan(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Canberra(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "stats", "pracma"), .export = c("Canberra", "canberra_generic", "querydata", "matrixzscore")) %dopar% {
            Canberra(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Chebyshev(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "stats", "pracma"), .export = c("Chebyshev", "chebyshev_generic", "querydata", "matrixzscore")) %dopar% {
            Chebyshev(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Dice(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "philentropy", "pracma"), .export = c("Dice", "dice_generic", "distance", "querydata", "matrixzscore")) %dopar% {
            Dice(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Jaccard(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "philentropy", "pracma"), .export = c("Jaccard", "jaccard_generic", "distance", "querydata", "matrixzscore")) %dopar% {
            Jaccard(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Mahalanobis(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "StatMatch", "pracma"), .export = c("Mahalanobis", "mahalanobis_generic", "mahalanobis.dist", "querydata", "matrixzscore")) %dopar% {
            Mahalanobis(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Cosine(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "philentropy"), .export = c("Cosine", "cosine_generic", "distance", "querydata", "matrixzscore")) %dopar% {
            Cosine(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Biweight(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "WGCNA"), .export = c("Biweight", "biweight_generic", "querydata", "matrixzscore")) %dopar% {
            Biweight(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Weighted_rank(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph"), .export = c("Weighted_rank", "wtd_rank_generic", "querydata")) %dopar% {
            Weighted_rank(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Phit(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "propr", "pracma"), .export = c("Phit", "phit_generic", "phit", "querydata", "matrixzscore")) %dopar% {
            Phit(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Phis(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "propr", "pracma"), .export = c("Phis", "phis_generic", "phis", "querydata", "matrixzscore")) %dopar% {
            Phis(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
        ...
        res.all <- Rhop(miRExp, mRExp, priori.information, p.value = p.value)        
	...
	res.single <- foreach(i = seq(nrow(miRExp)), .packages = c("igraph", "propr"), .export = c("Rhop", "rhop_generic", "perb", "querydata", "matrixzscore")) %dopar% {
            Rhop(miRExp[-i, ], mRExp[-i, ], priori.information, p.value = p.value)
        }     
